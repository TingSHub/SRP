...
static rt_err_t drv_pwm_set(TIM_HandleTypeDef *htim, struct rt_pwm_configuration *configuration)
{
    ...
    /* Convert nanosecond to frequency and duty cycle. 1s = 1 * 1000 * 1000 * 1000 ns */
    tim_clock /= 1000000UL; //84
    period = (unsigned long long)configuration->period * tim_clock / 1000ULL ; //84000
    psc = period / MAX_PERIOD + 1; //2
    period = period / psc; //42000
    __HAL_TIM_SET_PRESCALER(htim, psc - 1); //分频后，42M

    //PWM频率为1k，则period * 计时周期 = 1k
